    ...
    try {
      if (admin.apps.length === 0) {
        ...
        // Option 2: Full Service Account via JSON blob
          ...
          isFullAdmin = true;
          console.log(`[Firebase Admin] Initialized WITH Service Account (JSON) for project: ${serviceAccount.project_id || projectId}`);
        }
        // Option 3: Base Project ID (Limited functionality)
          ...
          isFullAdmin = false;
          console.warn(`[Firebase Admin] Initialized with Project ID ONLY. Actions requiring Auth/Service Account (like password reset/delete user) will fail.`);
        }
      } else {
        adminApp = admin.apps[0];
        // If it was already initialized, we don't know if it was full admin easily, but we assume it matched the first attempt
      }
      return { app: adminApp, isFullAdmin };
    } catch (e) {
      console.error("[Firebase Admin] Initialization fatal error:", e.message);
