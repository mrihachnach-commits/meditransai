    ...
    try {
      if (admin.apps.length === 0) {
        ...
        if (clientEmail && privateKey) {
          ...
          // Ensure headers/footers if partially missing
          ...
          isFullAdmin = true;
          console.log(`[Firebase Admin] Success: Initialized WITH Individual Vars for: ${projectId}`);
        } 
        ...
        // Option 3: Base Project ID (Limited functionality)
          ...
          isFullAdmin = false;
          console.warn(`[Firebase Admin] Warning: Initialized with Project ID ONLY. Auth tasks will fail on Vercel.`);
        }
      } else {
        // Try to find if we already have a full admin app
        adminApp = admin.apps.find(a => a.name === "full-admin" || a.name === "json-admin") || admin.apps[0];
        isFullAdmin = adminApp.name === "full-admin" || adminApp.name === "json-admin";
      }
