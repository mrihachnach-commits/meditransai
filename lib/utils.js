...
/**
 * @param {import('./firebaseAdmin.js').CustomRequest} req
 */
export const checkAdmin = async (req) => {
  try {
    ...
    if (!decodedToken) {
      ...
      if (verifyRes.status === 403 || (verifyRes.status === 400 && verifyData.error?.message?.includes("API_KEY"))) {
        throw { 
          status: 403, 
          error: "API Key không hợp lệ hoặc bị chặn bởi domain.", 
          details: "Vui lòng kiểm tra lại FIREBASE_API_KEY trong Vercel Environment hoặc Authorized Domains trong Firebase Console." 
        };
      }
